...
bool SocketClient::connect() {
    ...
    if (m_socketFd < 0) {
        LOG_ERROR("Failed to create socket: ", strerror(errno), " Socket creation failed.");
        return false;
    }
    ...
    if (::connect(m_socketFd, (struct sockaddr*)&addr, sizeof(addr)) == 0) {
        ...
        // Fallback to system socket only if primary socket doesn't exist or permission is denied
        if ((errno == ENOENT || errno == EACCES || errno == EPERM) &&
            ...
            m_socketFd = socket(AF_UNIX, SOCK_STREAM, 0);
            if (m_socketFd < 0) {
                LOG_ERROR("Failed to create socket for fallback: ", strerror(errno), " Socket creation failed for fallback.");
                return false;
            }
            ...
            if (::connect(m_socketFd, (struct sockaddr*)&addr, sizeof(addr)) == 0) {
                ...
                int systemSocketErrno = errno;
                LOG_DEBUG("Failed to connect to system socket: ", systemSocket, " error: ", strerror(systemSocketErrno));
            }
        } else if (!connected) {
            // Handle connection failure with good error messages for primary socket
            if (errno == EACCES || errno == EPERM || originalErrno == EACCES || originalErrno == EPERM) {
                LOG_ERROR("Permission denied connecting to socket: ", m_socketPath, ". Ensure daemon is running as the same user, or use system socket.");
                ...
                LOG_ERROR("Socket file does not exist or daemon is not running. Ensure cec-daemon is running.");
            }
...
Message SocketClient::receiveResponse() {
    ...
    if (bytesRead <= 0) {
        if (bytesRead == 0) {
            LOG_ERROR("Server closed connection while receiving response");
        } else {
            LOG_ERROR("Failed to receive response from server: ", strerror(errno));
        }
        return Message(MessageType::RESP_ERROR);
    }
