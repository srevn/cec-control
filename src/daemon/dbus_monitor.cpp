...
bool DBusMonitor::attach(EventLoop& loop) {
    ...
    if (!loop.add(busFd, m_registeredMask, [this](uint32_t) { this->onBusReadable(); })) {
        m_loop = nullptr;
        m_registeredBusFd = -1;
        return false;
    }
    if (!loop.add(m_timer.fd(), static_cast<uint32_t>(EventPoller::Event::READ),
